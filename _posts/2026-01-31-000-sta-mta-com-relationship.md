...
- [まず結論（ひとことで）](#まず結論ひとことで)
- [Apartment Modelの呼び出しパターン（図）](#apartment-modelの呼び出しパターン図)
  - [パターン1: 同一STAスレッド内での呼び出し](#パターン1-同一staスレッド内での呼び出し)
  - [パターン2: 同一MTA内での呼び出し](#パターン2-同一mta内での呼び出し)
  - [パターン3: Apartmentを跨ぐ呼び出し](#パターン3-apartmentを跨ぐ呼び出し)
- [STA（Single-Threaded Apartment）](#stasingle-threaded-apartment)
  - [なぜUIスレッドでSTAが使われるのか](#なぜuiスレッドでstaが使われるのか)
- [MTA（Multi-Threaded Apartment）](#mtamulti-threaded-apartment)
- [STA/MTAはどこで決まるのか](#stamtaはどこで決まるのか)
  - [.NETでのSTA/MTA](#netでのstamta)
- [STAを間違えると起きるハングの具体例](#staを間違えると起きるハングの具体例)
  - [よくある状況](#よくある状況)
  - [何が起きるのか](#何が起きるのか)
  - [擬似コード（典型的な失敗パターン）](#擬似コード典型的な失敗パターン)
  - [回避の要点](#回避の要点)
  - [「メッセージループを回す」って結局なに？](#メッセージループを回すって結局なに)
  - [正しい方向の例（雑に書くとこう）](#正しい方向の例雑に書くとこう)
  - [もう一つのハング例: 同期呼び出し中のコールバック](#もう一つのハング例-同期呼び出し中のコールバック)
- [ざっくり使い分け](#ざっくり使い分け)
- [まとめ](#まとめ)
- [参考資料](#参考資料)
