...
4. どこで起きたかで判断する
   1. UI イベント
   2. 1 件ずつ処理するジョブ / リクエスト
   3. 常駐ループ / 監視 / キュー処理
   4. 起動処理
   5. ネイティブ境界 / COM / P/Invoke / unsafe
5. 継続してよい条件
6. 終了したほうがよい条件
7. 典型パターン別のおすすめ
8. よくある NG
...
要するに、判断の軸は「catch できるか」ではなく、**不変条件（常に守られているべき前提）を回復できるか** です。
...
例外型だけで即決しないほうがよいです。  
同じ `FileNotFoundException` でも、ファイル選択画面で起きるのと、起動時に必須設定を読む場面で起きるのとでは重みが違います。
...
ここを見ずに「例外を捕まえたから継続」で済ませると、表面上だけ生きている妙なアプリができます。  
これはかなり厄介です。
...
大事なのは、**型名が怖いから終了** ではなく、**その例外が示している壊れ方が広いから終了** です。
...
UI では「アプリを落としたくない」という気持ちが強くなりがちですが、  
**壊れた状態で保存ボタンだけ押せてしまう** ほうが、だいたい被害が大きいです。
...
- 失敗単位が外から見て明確
- 途中変更がトランザクションや補償で整う
- 同じ処理をもう一度流しても結果が壊れない性質（idempotency）がある
- 失敗を隔離キュー（dead-letter）やエラー記録へ逃がせる

この条件があるなら、プロセス全体まで落とす必要はありません。

一方で、1 件処理に見えても、実際には「DB 更新 + ファイル出力 + 装置コマンド送信」が混ざっていることがあります。  
このとき、どこまで終わったか分からないなら、**次の 1 件へ元気に進む前に立ち止まる** ほうが安全です。
...
という半死半生の状態になります。かなり嫌なやつです。

ここでは方針を分けたほうがよいです。

- **各アイテム処理の境界** で想定内例外を捕まえる  
  例: 1 メッセージだけ失敗、1 ファイルだけ失敗
- **親ループ** から想定外例外が漏れたら、プロセス終了寄りにする  
  例: ワーカー全体の寿命管理が壊れた、共有状態が怪しい

特にサービスや長時間常駐アプリでは、**壊れたまま生かすより、サービスマネージャや親プロセスなどの supervisor に再起動させる** ほうが健全です。
...
半端に起動して一部メニューだけ動く、再現条件によってだけ落ちる、という状態は、利用者にも保守側にもつらいです。
...
このあたりは、managed 側で例外を見ても、**その前に native 側で何が壊れたか** が分からないことがあります。

特に次は終了寄りです。

- `AccessViolationException`
- ヒープ破壊やダブル free を疑う症状
- ハンドル異常、使い回し、解放後アクセスの臭い
- callback 境界で突然死ぬ
- 例外の前後で再現性が低く、症状が揺れる

こういう処理は、そもそも **別プロセスへ隔離する設計** が効きます。  
同じプロセスに危険物を全部積んでおいて「落ちないように祈る」は、かなり神頼みです。
...
ここでの継続は、**例外を無かったことにする** という意味ではありません。
...
- ネイティブ境界やメモリ破壊を疑う
- 以後の `finally` や finalizer まで信用しにくい
...
ここがあると、「落ちる = 終わり」ではなくなります。

なお、`Environment.FailFast` は「とりあえず終了」の道具ではありません。  
**これ以上 `finally` や finalizer を回すのも危ない** と判断したときに使う、かなり強い終了手段です。
...
`AppDomain.UnhandledException`、`Application.ThreadException`、`DispatcherUnhandledException` などは、  
**最後に記録する場所** としては有用ですが、**魔法の回復ポイント** ではありません。

### 8.3. 外部副作用があるのに安易に retry する

装置コマンド、メール送信、課金、ファイル移動、DB 更新などで同じ処理の再実行安全性がないまま retry すると、  
今度は二重実行事故が主役になります。

### 8.4. 監視ループが死んだのに UI だけ残す

見た目だけ生きていて、仕事をしていないアプリはかなり迷惑です。  
サービスや常駐アプリでは特に避けたい形です。
...
ここがないまま「とにかく落とさない」は、設計ではなく願望です。
...
ここを分けないと、全部同じ `catch (Exception)` に流れ込みます。
...
COM / ActiveX / vendor SDK / unsafe / 重い画像処理 / 外部機器制御など、  
落ちたときの被害を広げたくないものは、別プロセス化がかなり効きます。
...
この答えが曖昧なら、`Handled = true` で押し切るより終了寄りです。  
`AppDomain.UnhandledException` も、基本は最後の記録地点として考えるほうが安全です。

## 10. まとめ

想定していない例外が起きたときに見るべきなのは、  
「この例外は catch できるか」ではなく、  
**このあともアプリの状態を信用できるか** です。
...
特に、長時間動くアプリ、監視アプリ、サービス、装置連携では、  
**壊れたまま生きる** ことのほうが、**素直に落ちる** ことより危険な場面がかなりあります。
...
- [C# async/await のベストプラクティス - Task.Run と ConfigureAwait の判断表](https://comcomponent.com/blog/2026/03/09/001-csharp-async-await-best-practices/)
- [Generic Host / BackgroundService をデスクトップアプリに持ち込む理由 - 起動・寿命・graceful shutdown の整理がかなり楽になる](https://comcomponent.com/blog/2026/03/12/002-generic-host-backgroundservice-desktop-app/)
- [COM STA/MTA の基礎知識 - スレッドモデルとハングを避ける考え方](https://comcomponent.com/blog/2026/01/31/000-sta-mta-com-relationship/)
- [産業用カメラ制御アプリが1か月後に突然落ちるとき（前編） - ハンドルリークの見つけ方と長期稼働向けログ設計](https://comcomponent.com/blog/2026/03/11/002-handle-leak-industrial-camera-long-run-crash-part1/)
...
- [プロフィールを見る](https://comcomponent.com/profile/go-komura/)
- [KomuraSoft トップ](https://comcomponent.com/)
