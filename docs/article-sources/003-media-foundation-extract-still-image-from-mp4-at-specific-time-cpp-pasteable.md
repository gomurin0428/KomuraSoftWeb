...
以下の 1 ブロックが、そのまま Visual Studio の C++ コンソールアプリプロジェクトへ持っていく前提のコードです。コマンドライン引数は `input.mp4`、`seconds`、`output.png` の順です。記事はここで終わりなので、**最後のこのコードブロックだけ抜けば動く** 形にしてあります。
