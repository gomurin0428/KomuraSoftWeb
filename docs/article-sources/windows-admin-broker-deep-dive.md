...
この記事は、以前書いた「Windowsアプリ開発における最低限のセキュリティを守るためのチェックリスト」のうち、`asInvoker` を基本にして、管理者権限が必要な処理だけを分離する、という話を掘り下げたものです。
...
のように **操作自体を固定**し、  
必要な引数も **bool / enum / 数値 / 限定された文字列** に寄せます。
...
ランダムな pipe 名だけでもだいぶマシですが、同じユーザーで動く別プロセスが先に接続する余地はゼロではありません。  
...
UI は「Explorer の右クリックメニューに登録する」のチェックボックスだけ持ち、  
実際のレジストリ操作は helper 側で行います。
...
public sealed class ElevationBrokerClient
{
    ...
    private static string BuildArguments(string pipeName, int clientPid, string clientSid)
    {
        return string.Create(
            ...
            $"--pipe {pipeName} --client-pid {clientPid} --client-sid {clientSid}");
    }
...
- helper EXE の path を **絶対パス**で持つ
- `UseShellExecute = true` を**明示**する
- `Verb = "runas"` を**明示**する
- helper に渡すのは **pipe 名 / 呼び出し元 PID / 呼び出し元 SID** だけにする

UI から helper に「実行したいレジストリコマンド全文」みたいなものは渡しません。  
helper に渡る操作本体は、pipe の中で送る **型付き request** だけです。
...
internal static class Program
{
    ...
    {
        SecurityIdentifier brokerSid = WindowsIdentity.GetCurrent().User
            ?? throw new InvalidOperationException("Broker user SID could not be resolved.");

        var clientSid = new SecurityIdentifier(options.ClientUserSid);

        var pipeSecurity = new PipeSecurity();
        pipeSecurity.SetOwner(brokerSid);
    ...
    {
        if (!GetNamedPipeClientProcessId(pipe.SafePipeHandle.DangerousGetHandle(), out uint actualClientProcessId))
        {
        ...
        {
            throw new InvalidOperationException(
                $"Unexpected pipe client process. Expected={expectedClientProcessId}, Actual={actualClientProcessId}");
        }
    }
    ...
    {
        SetExplorerContextMenuRequest request =
            payload.Deserialize<SetExplorerContextMenuRequest>(BrokerJson.Options)
            ?? throw new InvalidDataException("Payload could not be deserialized.");

        ExplorerContextMenuRegistration.Apply(request.Enabled);

        return Task.FromResult(BrokerResponse.Ok("Explorer context menu setting has been updated."));
    }
...
internal static class ExplorerContextMenuRegistration
{
    ...
    {
        string fullPath = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, ClientExecutableName));

        if (!File.Exists(fullPath))
        {
            throw new FileNotFoundException("The client executable was not found.", fullPath);
        }
...
これが大事です。  
helper を「レジストリ編集代行ツール」にした瞬間、設計の温度が急に上がります。
...
public partial class SettingsPage
{
    ...
    private async void ExplorerMenuCheckBox_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            MessageBox.Show("設定を更新しました。", "MyApp");
        }
        catch (OperationCanceledException)
        {
            MessageBox.Show("管理者権限の承認がキャンセルされました。", "MyApp");
            ExplorerMenuCheckBox.IsChecked = !enabled;
        }
        catch (Exception ex)
        {
            MessageBox.Show(ex.Message, "設定変更に失敗しました");
            ExplorerMenuCheckBox.IsChecked = !enabled;
        }
    }
}
...
## 17. ファイル操作 / service / firewall に置き換えるときの考え方

今回の題材は registry でしたが、他の管理者処理でも考え方は同じです。

### 17.1 ファイル操作

たとえば Program Files 配下に machine-wide の設定ファイルを書くなら、helper には次だけを受けさせます。
...
service のインストール / アンインストール / start / stop でも同じです。  
UI から渡すのは、
...
のような固定 operation だけにします。  
`sc.exe create ...` の全文を渡してはいけません。
...
くらいまで絞ったほうがよいです。  
UI から `netsh advfirewall ...` の全文を渡すのは、だいぶワイルドです。

---

## 18. service / task / COM を選ぶ目安

ここまで broker EXE を中心に書きましたが、ずっとこれ一択ではありません。

### 18.1 service に切り替える目安
...
この場合は、最初から service のほうが整理しやすいです。
...
普通のデスクトップアプリなら、最初から COM 昇格モニカーに飛びつかないほうが楽です。
...
- Launching Applications (ShellExecute, ShellExecuteEx, SHELLEXECUTEINFO)  
  https://learn.microsoft.com/ja-jp/windows/win32/shell/launch
