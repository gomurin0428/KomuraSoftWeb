...
slug: windows-app-security-minimum-checklist
---

# Windowsアプリ開発における最低限のセキュリティを守るためのチェックシート

2026年03月14日 15:00 · Go Komura · Windows開発, セキュリティ, 設計, C# / .NET
...
この記事では、Windows アプリ開発で最低限守りたいセキュリティを、設計・実装・配布・運用の順にチェックしやすい形で整理します。  
全部を一気に完璧にする話ではありません。まずは、明らかに危ない穴を残さないことを狙います。
...
- 最低限として最初に外したくないのは、**不要な管理者権限を要求しないこと、署名すること、秘密情報を平文で持たないこと、証明書検証を無効化しないこと**です。
- Windows アプリは、**配布物そのもの**が攻撃面になります。EXE / DLL / MSI / MSIX / 自動更新モジュールまで含めて見たほうが安全です。
...
- Windows で配布するアプリは、**署名 + タイムスタンプ**を前提に考えたほうがよいです。利用者への信頼性だけでなく、改ざん検知や運用の説明もしやすくなります。
- 保存時の機密情報は、用途に応じて **DPAPI / ProtectedData** や **Credential Locker** を使い分けます。少なくとも「`appsettings.json` に平文で置く」は卒業したいところです。
- ログは多いほどよいわけではありません。**トークン、パスワード、接続文字列、個人情報、フルリクエスト本文**をそのまま残すと、ログ自体が事故の主役になります。

要するに、最低限のセキュリティは「特殊な機能を足すこと」より、**危ない既定動作や雑な実装を残さないこと**です。
...
この節は、まず全体をざっと見るための節です。  
細かい議論の前に、次の表だけ見れば見直す場所のあたりは付きます。
...
アプリ全体を管理者権限で動かすと、バグや DLL すり替え、設定ファイル誤読、外部入力の不備がそのまま強い権限で実行されます。
...
ここでのポイントは、「管理者で動けば楽」はだいたい後で効いてくることです。  
最小権限で動かして、それでも必要な操作だけ切り出したほうが、事故の半径がかなり小さくなります。
...
一方で、独自アップデータを持つなら、**通信の安全性**と**配布物の真正性**の両方を確認する必要があります。HTTPS だけでは「通信経路」は守れても、「そのファイルが本当に自分の発行物か」までは保証しません。ここは署名確認まで含めて考えたいです。

### 3.5. 秘密情報をソースコードや平文設定に置かない

ここは、実務だと本当に事故りやすいところです。  
...
どうしても接続文字列に資格情報を含めるなら、少なくとも `Persist Security Info=False` を維持し、平文の設定ファイルへ置きっぱなしにしないほうが安全です。
...
証明書検証を外すと、HTTPS を使っていても、中身はかなり骨抜きです。
...
特に、カレントディレクトリや PATH に依存した読み込みは、後からかなり嫌な事故になります。
...
最近の .NET では redaction（秘匿情報のマスキング）を前提にした整理もしやすくなっています。  
...
最後に、リリース前に最低限見たい項目をそのまま並べます。  
レビューや出荷判定の雛形として使いやすいように、Yes / No で確認できる形にしています。
...
権限設計、配布方式、更新方式、ログ設計を含む Windows ソフトの新規開発や改修に関わるテーマです。

### [技術相談・設計レビュー](https://comcomponent.com/services/technical-consulting/)

既存アプリのセキュリティ見直し、権限境界の再設計、アップデータ方針、既存資産を活かした安全化の整理に向いています。各ページの問い合わせ導線から連絡できます。
