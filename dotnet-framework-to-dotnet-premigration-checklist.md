...
slug: dotnet-framework-to-dotnet-premigration-checklist
...
date: 2026-03-15T10:30:00+09:00
author: 小村 豪
tags:
  - .NET
  - .NET Framework
...
.NET Framework の現場には、`System.Web`、WCF、Web Forms、古い `packages.config`、`web.config.install.xdt`、ネイティブ DLL、COM / ActiveX、設計時だけ動くサードパーティ製コントロール、暗黙の `x86` 前提、デザイナー依存の ResX、古いシリアライザーなど、**「普段は意識していない前提」** がかなり眠っています。

なので、.NET Framework から .NET への移行で本当に大事なのは、実装に入る前の棚卸しです。  
着手前に論点を分解できていれば、移行は「大きな賭け」ではなく「順番に潰す作業」になります。

この記事では、既存の **.NET Framework 4.x の業務アプリケーション** を **現在の .NET** に移行する前に、何を確認しておくべきかを整理します。  
対象は主に以下です。
...
執筆時点は **2026-03-15** です。サポート期間や公式ツールの推奨は変わるので、そこは常に最新情報で見てください。
...
- **WinForms / WPF は .NET に移行できても Windows 専用のまま** です。ここを勘違いすると「移行したのに Linux コンテナに載らない」という、よくある地雷を踏みます。
...
逆に、アプリが以下に強く依存しているなら、**当面は .NET Framework 4.8.1 に寄せて安定運用しつつ、別ラインで置き換え計画を立てる** のは合理的です。
...
大事なのは、**「移行したい」ではなく「移行後にどこへ着地したいか」** を先に決めることです。
...
- 「既存ライブラリの都合で 1 つ前の LTS にしたい」は、事情としてはあり得るが、**いつまで保守されるのか** を日付で見て判断する
...
ここを決めずに移行を始めると、途中で「Windows 固定で良かったのか」「いや、コンテナに載せたかったんだった」と話がねじれます。
...
Microsoft の移植前ガイドは、かなり実務的です。  
要するに、**移行前に今の .NET Framework プロジェクトを modern な入口に寄せておけ** という話です。

### 4.1 .NET Framework 4.7.2 以降へ上げる

公式ガイドでは、移植前に **.NET Framework 4.7.2 以降** をターゲットにすることが推奨されています。  
理由は、.NET Standard が既存 API をそのまま持っていないときでも、より新しい API 代替に寄せやすくなるからです。
...
つまり、**「パッケージ形式を変えるだけ」と思わないほうがよい** です。  
...
2026-03 時点では、**.NET Upgrade Assistant は正式に非推奨** で、Microsoft の現在の公式推奨は **GitHub Copilot モダン化** です。  
これは評価、計画、コード修正、検証までを支援する流れになっています。

ただし、現行ドキュメントでは **Visual Studio 2026 / Visual Studio 2022 17.14.17 以降**、**GitHub Copilot**、そして **C# コード** が前提として書かれています。
...
**ビジネスロジックだけを切り出せるなら軽い、アプリモデルまで抱え込んでいると重い**、という見方をすると分かりやすいです。
...
つまり、ASP.NET Framework のアプリでやるべき確認は、
...
なので、**「WCF を呼ぶ側」だけなら、見た目ほど重くない** ケースがあります。
...
たとえば、WPF のような app model 固有 API を前提にしたライブラリは、普通に難しいです。  
つまり、**shared library と言っても、本当に shared できる責務だけに絞る** のが重要です。
...
.NET Framework 時代に `AnyCPU` で動いていたように見えても、実際には
